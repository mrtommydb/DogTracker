...
void loop() {
  ...
  gpsConverted1[119] = 0;
  gpsConverted2[119] = 0;
  gpsConverted3[119] = 0;//Add a null value to the end of the char array. It needs a null character to end.
  Serial.println("gpsConverted");
  Serial.println(converted); //For debugging, print the selected data
  Serial.println(gpsConverted1);
  Serial.println(gpsConverted2);
  Serial.println(gpsConverted3);
  //The following commented out section is part of the old code that would send an sms to a predefined #. It has been commented out for the purpose of debugging the auto reply.
  /*
  ...
  flushSerial();
  if (!fona.sendSMS(sendto, converted)) { //Sends out the signal. Replace gpsdata with message and uncomment the array above if you want to send your own message for debugging.
      Serial.println(F("Failed to send SMS"));
    } else {
      Serial.println(F("Sent!"));
  }
  */
  //The following commented out section is the groundwork for the loop that would have the device store different sets of coordinates and send them in a group.
  //It would use the above code to send. However, due to the change in how the cooridnates are stored, this needs reworking before it can be implemented.
  /*
  if (iteration = (repeatBeforeReport - 1)) { //If we have reached the desired # loops/stored coordinates
  for (int i = 0; i < repeatBeforeReport; i++) {//Send all stored data
  ...
  */
  ...
  if (fona.available())      //any data available from the FONA?
  {
    ...
    *bufPtr = 0;
    //Scan the notification string for an SMS received notification. If it's an SMS message get the slot number in 'slot'
    if (1 == sscanf(fonaNotificationBuffer, "+CMTI: " FONA_PREF_SMS_STORAGE ",%d", &slot)) {
      ...
      if (!fona.sendSMS(callerIDbuffer, converted)) {
